...
// default authority
const DEFAULT_AUTHORITY = "https://login.live.com/oauth20_authorize.srf?";
...
export class UserAgentApplication {
  ...
  protected authorityInstance: string;
   ...
   */
  public getAuthorityInstance(): string {
    return this.authorityInstance;
  }
  ...
  acquireTokenRedirect(request: AuthenticationParameters): void {
    ...
    const acquireTokenAuthority = this.authorityInstance;

    // Track the acquireToken progress
    this.acquireTokenInProgress = true;

      const responseType = ResponseTypes.token;
    ...
    return new Promise<AuthResponse>((resolve, reject) => {
      ...
      // else proceed with login
        ...
        if (!serverAuthenticationRequest.authorityInstance) {
            serverAuthenticationRequest.authorityInstance = request.authority ? request.authority : this.authorityInstance;
        }
        // cache miss
        try {
  ...
  logout(): void {
    ...
    const urlNavigate = this.getPostLogoutRedirectUri();
    this.promptUser(encodeURIComponent(urlNavigate));
  }
  ...
  protected clearCache(): void {
    window.renewStates = [];
    const accessTokenItems = this.cacheStorage.getAllAccessTokens(Constants.clientId, Constants.userId);
  ...
  protected getResponseState(hash: string): ResponseStateInfo {
    ...
    if (stateResponse.state === this.cacheStorage.getItem(Constants.stateAcquireToken, this.inCookie)) { //acquireTokenRedirect
  ...
  private getCachedToken(serverAuthenticationRequest: ServerRequestParameters, account: Account): AuthResponse {
    ...
    const tokenCacheItems = this.cacheStorage.getAllAccessTokens(this.clientId, account ? account.userId : null);
    ...
    if (!serverAuthenticationRequest.authority) {
      ...
      if (filteredItems.length === 1) {
        ...
        serverAuthenticationRequest.authorityInstance = accessTokenCacheItem.key.authority;
      }
      ...
      // if no match found, check if there was a single authority used
        ...
        serverAuthenticationRequest.authorityInstance = authorityList[0];
      }
    }
  ...
  private renewToken(scopes: Array<string>, resolve: Function, reject: Function, account: Account, serverAuthenticationRequest: ServerRequestParameters): void {
    ...
    let urlNavigate = serverAuthenticationRequest.createNavigateUrl(scopes) + Constants.prompt_none + Constants.response_mode_fragment;
  ...
  private saveAccessToken(response: AuthResponse, authority: string, parameters: any): AuthResponse {
    let scope: string;
    ...
    // if the response contains "scope"
      ...
      for (let i = 0; i < accessTokenCacheItems.length; i++) {
        ...
        if (accessTokenCacheItem.key.userId === response.account.userId) {
      ...
      const accessTokenKey = new AccessTokenKey(authority, this.clientId, scope, parameters["user_id"]);
      const accessTokenValue = new AccessTokenValue(parameters[Constants.accessToken], expiresIn, parameters["user_id"]);
  ...
  protected saveTokenFromHash(hash: string, stateInfo: ResponseStateInfo): AuthResponse {
    ...
    if (hashParams.hasOwnProperty(Constants.errorDescription) || hashParams.hasOwnProperty(Constants.error)) {
      ...
      if (stateInfo.requestType === Constants.renewToken) {
        ...
        if (account && !Utils.isEmpty(account.userId)) {
    ...
    // If the server returns "Success"
      ...
      if (stateInfo.stateMatch) {
        ...
        if (hashParams.hasOwnProperty(Constants.accessToken)) {
          ...
          response.account = Account.createAccount(hashParams["user_id"]);

          let accountKey: string;
          if (response.account && !Utils.isEmpty(response.account.userId)) {
            accountKey = response.account.userId;
          }
          else {
          ...
          if (!Utils.isEmpty(cachedAccount)) {
            ...
            if (response.account && acquireTokenAccount && Utils.compareAccounts(response.account, acquireTokenAccount)) {
              response = this.saveAccessToken(response, authority, hashParams);
  ...
  getAccount(): Account {
    ...
    const userId = this.cacheStorage.getItem(Constants.userId);
    // const rawClientInfo = this.cacheStorage.getItem(Constants.msalClientInfo);

    if (!Utils.isEmpty(userId)) {
      this.account = Account.createAccount(userId);
      return this.account;
    }
  ...
  getAllAccounts(): Array<Account> {
    ...
    const accessTokenCacheItems = this.cacheStorage.getAllAccessTokens(Constants.clientId, Constants.userId);

    for (let i = 0; i < accessTokenCacheItems.length; i++) {
      const userId = accessTokenCacheItems[i].value.userId;
      const account: Account = Account.createAccount(userId);
  ...
  private getUniqueAccounts(accounts: Array<Account>): Array<Account> {
    ...
    for (let index = 0; index < accounts.length; ++index) {
      if (accounts[index].userId && flags.indexOf(accounts[index].userId) === -1) {
        flags.push(accounts[index].userId);
  ...
  protected getCachedTokenInternal(scopes : Array<string> , account: Account, state: string): AuthResponse {
    ...
    const newAuthority = this.authorityInstance ? this.authorityInstance : this.authority;
    const responseType = ResponseTypes.token;
  ...
  private getAccountId(account: Account): any {
    ...
    if (!Utils.isEmpty(account.userId)) {
         accountId = account.userId;
    }
    else {
        accountId = Constants.no_account;
  ...
  //   let eQParams = request.extraQueryParameters;
  //   // if (request) {
